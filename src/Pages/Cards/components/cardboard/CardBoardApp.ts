...
export interface CardBoardAppCallbacks {
  ...
  onContextMenu?: (event: { x: number; y: number; cell: CellCoord; cardId: string | null }) => void;
...
export class CardBoardApp {
  ...
  constructor(wrapper: HTMLElement, callbacks: CardBoardAppCallbacks = {}) {
    ...
    this.viewport = { width: wrapper.clientWidth, height: wrapper.clientHeight };
    this.cards = readCards();
    this.rebuildOccupancy();
    this.grid = new GridRenderer();
    this.cardRenderer = new CardRenderer();
    this.connectionRenderer = new ConnectionRenderer();
    this.app.stage.addChild(this.grid.container, this.connectionRenderer.container, this.cardRenderer.container);
    this.bindEvents();
    this.app.ticker.add(this.tick);
  }

  setMoveMode(cardId: string | null): void { this.moveCardId = cardId; this.markDirty(); }
  ...
  deleteCard(cardId: string): void {
    const card = this.cards[cardId];
    if (!card) return;
    for (const other of Object.values(this.cards)) other.connects = other.connects.filter((connection) => connection.id !== cardId);
    delete this.cards[cardId]; this.rebuildOccupancy(); this.save(); this.markDirty();
  }
  ...
  private onPointerDown = (event: PointerEvent): void => {
    ...
    if (this.disconnectSourceId) { if (card?.id === this.disconnectSourceId) return this.cancelModes(); if (card) this.disconnect(card); return; }
    if (this.connectSourceId) { if (card?.id === this.connectSourceId) return this.cancelModes(); if (card) this.connect(card); return; }
    if (this.moveCardId) { if (card && card.id === this.moveCardId) return; const moving = this.cards[this.moveCardId]; if (moving && checkPlacement(this.occupancy, cell, moving.width, moving.height, moving.id).available) { this.moveCard(moving, cell); } return; }
  ...
  private onResize = (): void => { this.viewport.width = this.wrapper.clientWidth; this.viewport.height = this.wrapper.clientHeight; this.app.renderer.resize(this.viewport.width, this.viewport.height); this.markDirty(); };

  private connect(target: CardEntity): void { const source = this.cards[this.connectSourceId!]; if (!source || source.connects.some((connection) => connection.id === target.id)) return; source.connects.push({ id: target.id, color: this.connectColor }); target.connects.push({ id: source.id, color: this.connectColor }); this.save(); this.cancelModes(); }
  private disconnect(target: CardEntity): void { const source = this.cards[this.disconnectSourceId!]; if (!source) return; source.connects = source.connects.filter((connection) => connection.id !== target.id); target.connects = target.connects.filter((connection) => connection.id !== source.id); this.save(); this.cancelModes(); }
  private moveCard(card: CardEntity, cell: CellCoord): void { card.coordinates = cell; card.cells = getCardCells(cell, card.width, card.height); this.rebuildOccupancy(); this.save(); this.moveCardId = null; this.markDirty(); }
  private cancelModes(): void { this.moveCardId = null; this.connectSourceId = null; this.disconnectSourceId = null; this.hoveredCardId = null; this.markDirty(); }
